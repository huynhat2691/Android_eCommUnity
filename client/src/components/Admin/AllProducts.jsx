...
const AllProducts = () => {
  ...
  const renderStatusSelect = (item) => {
    ...
    return (
      ...
      >
        <SelectTrigger
          className={`w-[180px] !bg-white !text-black ${currentColors.bg} ${currentColors.text} ${currentColors.border} border-2`}
        >
          <SelectValue>{statusToVietnamese[item.status]}</SelectValue>
        </SelectTrigger>
        <SelectContent className={`${currentColors.border} border-2 !bg-white !text-black`}>
  ...
  return (
    <Card className="w-full mx-auto h-[unset] !rounded-none !border-none !bg-white !text-black">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-[16px] ">Quản lý sản phẩm</CardTitle>
          <form
            onSubmit={handleSearch}
            className="flex w-[60%] max-w-sm items-center space-x-2"
          >
            <Input
              type="text"
      ...
      <Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen} >
        <DialogContent className=" !bg-white !text-black h-screen">
          <DialogHeader >
            <DialogTitle className="text-center flex items-center justify-center">Xem trước sản phẩm</DialogTitle>
          </DialogHeader>
