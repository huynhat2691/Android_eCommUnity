...
const ChatPopup = () => {
  ...
  if (!isVisible && !isOpen) {
    return (
      <Button
        className="fixed bottom-16 right-3  !bg-[#27b3e2] !text-white rounded-md shadow-lg "
        onClick={() => dispatch(toggleChatPopup())}
      >
        <div className="font-[500] flex items-center">
          <MessageSquare strokeWidth={3} size={20} />
  ...
  return (
    <div className="fixed bottom-16 right-0 z-20 w-screen md:h-[600px] bg-white shadow-xl rounded-lg flex flex-col overflow-hidden transition-all duration-300 ease-in-out ">
      <div className="p-1 px-4 bg-[#27b3e2] text-white rounded-t-md flex justify-between items-center">
        <h3 className="font-bold">Chat</h3>
        <Button
          variant="ghost"
              ...
              <form onSubmit={handleSendMessage} className="p-2 border-t">
                ...
                <div className="flex items-center">
                  <Input
                    ...
                    value={newMessage}
                    onChange={(e) => setNewMessage(e.target.value)}
                    className="flex-grow mr-2 !border !bg-white !text-black "
                    placeholder="Nhập nội dung tin nhắn"
                  />
                  <Input
                    type="file"
