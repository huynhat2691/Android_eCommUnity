...
const UserAllOrders = () => {
  ...
  const renderOrderCards = (filteredOrders) => {
    if (!filteredOrders || filteredOrders.length === 0) {
      return (
        <div className="w-full min-h-screen mx-auto m-4">
          <CardContent>
    ...
    return (
      <div className="grid grid-cols-1">
        {filteredOrders.map((order) => (
          <Card key={order._id} className="p-4 my-2 !bg-white !text-black">
            {order.cart.map((item) => (
    ...
    <div className="container mx-auto">
      ...
      <Tabs defaultValue="all" onValueChange={setActiveTab}>
        <div className="w-full overflow-x-auto scrollbar-hide">
          <TabsList className="inline-flex min-w-full !bg-white !text-black">
            <TabsTrigger value="all" className="flex-shrink-0 min-w-[120px]">
              Tất cả đơn hàng
            </TabsTrigger>
            <TabsTrigger
              value="Processing"
            ...
            <TabsTrigger value="Refund" className="flex-shrink-0 min-w-[120px]">
              Trả hàng/Hoàn tiền
            </TabsTrigger>
          </TabsList>
        </div>
