...
const OrderDetails = () => {
  ...
  return (
    <Card className="w-full mx-auto flex flex-col min-h-[calc(100vh-6rem)] !bg-white !text-black !border-none !rounded-none">
      <CardHeader>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center">
            <ClipboardList size={25} color="red" />
            <CardTitle className="pl-2 text-xl sm:text-2xl">
              Chi tiết đơn hàng
            </CardTitle>
          </div>
          <Link to="/dashboard/orders">
            <Button variant="outline" className="!bg-[#27b3e2] !text-white !border-none">Danh sách đơn hàng</Button>
          </Link>
        </div>
      </CardHeader>
      ...
      <CardContent className="flex flex-col !justify-between h-full">
              ...
              <div key={index} className="flex items-start">
                ...
                <div className="ml-3">
                  <div className="flex flex-wrap items-center gap-1">
                    {item.isUsed && (
                      <div>
                        <p className="text-xs border-[red] border font-semibold text-[red] px-[2px]">
                          Hàng cũ
                        </p>
                      </div>
                    )}
                  ...
                  <p className="text-sm text-muted-foreground">
                    {item.hasClassifications
                      ? `${item.selectedClassification.group1} - ${item.selectedClassification.group2}`
                      : "Không có phân loại"}
                  </p>
                </div>
              </div>
        ...
        <div className="mt-8 border-t p-4 flex items-center justify-end text-base sm:text-lg">
          <p className="mr-2 font-[500]">Tổng cộng:</p>
          {data?.totalPrice}
          <sup>₫</sup>
        </div>

        <div className="m-4 sm:m-10 flex flex-col space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            {status === "Cancelled" ? (
              <div className="w-full sm:w-[200px] p-2 border rounded bg-gray-100">
                <span className="text-red-500">Đã hủy</span>
              </div>
            ) : status !== "Processing Refund" &&
              status !== "Refund Success" ? (
              <div className="flex flex-col sm:flex-row gap-3 w-full">
                <Select value={status} onValueChange={setStatus}>
                  <SelectTrigger className="w-full sm:w-[300px] !bg-white !text-black">
                    <SelectValue placeholder={getStatusInVietnamese(status)} />
                  </SelectTrigger>
                  <SelectContent>
                    {[
                ...
                {data?.status !== "Delivered" &&
                  ...
                  data?.status !== "Cancelled" && (
                    <Button
                      className="w-full sm:w-auto !bg-[#27b3e2] !text-white"
                      onClick={
                        data?.status !== "Processing Refund"
                          ? orderUpdateHandler
                          : refundOrderUpdateHandler
